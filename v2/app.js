...
(function () {
  ...
  function apiErrorText(error) {
    if (error === 'network_error') return 'The V2 backend is not reachable. Start the local backend, check the API URL, then try again.';
    if (error === 'request_timeout') return 'The V2 backend did not respond in time. Try again after the backend is healthy.';
      ...
      return h('section', { class: 'arc' }, [
        ...
        h('div', { class: 'lesson-grid' }, inArc.map(l => {
          ...
          return h('a', {
            ...
            h('span', { class: 'lt-action' + (locked ? ' locked' : '') + (current ? ' next' : '') }, locked ? 'Locked' : done ? 'Review lesson' : current ? 'Open next lesson' : 'Open lesson')
          ]);
        }))
      ]);
    }).filter(Boolean);
