...
assertIncludes('coolify-backend/db.mjs', 'async function curriculum()');
...
assertIncludes('coolify-backend/db.mjs', 'SELECT count(*)::int AS count FROM lesson_steps WHERE lesson_id = $1');
assertIncludes('coolify-backend/db.mjs', 'ON CONFLICT (lesson_id, step_index) DO NOTHING');
...
assertIncludes('coolify-backend/test-server.mjs', 'Postgres integration skipped: DATABASE_URL is not set.');

const runRouteTests = String(process.env.RUN_BACKEND_ROUTE_TESTS || '').toLowerCase() === 'true';
if (runRouteTests) {
  run('node', ['coolify-backend/test-server.mjs']);
}
