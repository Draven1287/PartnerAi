...
function originSet(value, fallback) {
  return new Set(String(value || fallback.join(',')).split(',').map(origin => origin.trim()).filter(Boolean));
}
...
function allowedOriginsForRequest(req) {
  return isAdminRequest(req) ? ADMIN_ALLOWED_ORIGINS : ALLOWED_ORIGINS;
}

function corsOrigin(req) {
  const origin = req?.headers?.origin;
  if (!origin) return '';
  return allowedOriginsForRequest(req).has(origin) ? origin : '';
}
...
function median(numbers) {
  const values = numbers.map(Number).filter(Number.isFinite).sort((a, b) => a - b);
...
function answerRank(value) {
  const number = Number(value);
  if (!Number.isFinite(number)) return null;
  return Math.max(0, Math.min(3, Math.round(number)));
}

function buildAdminOverview({ learners = [], analytics = {}, visits = null } = {}) {
  ...
  for (const attempt of attempts) {
    if (attempt.userId && !latestAttemptByUser.has(attempt.userId)) latestAttemptByUser.set(attempt.userId, attempt);
  }
  ...
  const normalizedLearners = learners.map(row => {
    ...
    return {
      ...
      age_range: attempt.ageRange || 'unknown',
      level: consoleLevel(attempt.level),
      score: Number.isFinite(Number(attempt.scorePercent)) ? Number(attempt.scorePercent) : 0,
      current_lesson: lesson,
      visits: Number(row.visitCount || 0),
      active_days: Number(row.activeDays || 0),
      last_seen: row.lastActiveAt || row.createdAt || ''
    };
  });

  const levels = ['Foundation', 'Explorer', 'Builder'].map(level => ({
    level,
    n: normalizedLearners.filter(row => row.level === level).length
  }));

  const questionOrder = ['definition', 'capability', 'limits', 'learning', 'impact', 'systems'];
  const questions = Object.fromEntries(questionOrder.map(key => [key, [0, 0, 0, 0]]));
  for (const response of responses) {
    const key = String(response.questionKey || '').trim();
    if (!questions[key]) continue;
    const rank = answerRank(response.score ?? response.selectedValue);
    if (rank == null) continue;
    questions[key][rank] += 1;
  }

  const total = normalizedLearners.length;
  const activeWeek = normalizedLearners.filter(row => daysSince(row.last_seen) <= 7).length;
  const newThisWeek = learners.filter(row => daysSince(row.createdAt) <= 7).length;
  const lessonValues = normalizedLearners.map(row => row.current_lesson);
  const avgLesson = lessonValues.length
    ? Math.round((lessonValues.reduce((sum, value) => sum + value, 0) / lessonValues.length) * 10) / 10
    : 0;
  ...
  return {
    ...
    newThisWeek,
    avgLesson,
    medianVisits: Math.round(median(normalizedLearners.map(row => row.visits))),
    medianActiveDays: Math.round(median(normalizedLearners.map(row => row.active_days))),
    levels,
...
async function handleAdminLogout(req, res, db, { url } = {}) {
  const session = await requireAdmin(req, res, db, { url, csrf: true });
  if (!session) return;
  await db.deleteSession(session.session.token_hash);
...
async function handleV1Minutes(req, res, db) {
...
function learnerCsv(rows) {
  return ['email,displayName,totalMinutes,visitCount,currentLesson,completionPercent,lastActiveAt,savedCardCount,savedNoteCount,nameChangeCount,lastNameChangedAt', ...rows.map(row => [
    row.email,
    row.displayName,
    row.totalMinutes,
    row.visitCount,
    ...
    row.lastActiveAt || '',
    row.toolkitCards || 0,
    row.savedNoteCount || 0,
    row.nameChangeCount || 0,
    row.lastNameChangedAt || ''
  ].map(csvEscape).join(','))].join('\n');
}

function assessmentResponsesCsv(rows) {
  return ['email,displayName,ageRange,level,scorePercent,category,questionKey,selectedValue,selectedLabel,score,freeText,completedAt', ...rows.map(row => [
    row.email,
    row.displayName,
    row.ageRange,
    row.level,
...
<head>
  ...
  <style>
    :root { --bg:#f5f8f6; --surface:#fff; --surface-2:#eaf2ee; --border:#d7e2dc; --text:#111827; --dim:#46566a; --faint:#718096; --accent:#0f8a66; --accent-soft:#e5f4ef; --good:#0f8a66; --bad:#b42318; --warn:#d97706; }
    * { box-sizing:border-box; }
    ...
    .shell { display:grid; grid-template-columns:minmax(190px,230px) minmax(0,1fr); min-height:100vh; }
    aside { background:var(--surface); border-right:1px solid var(--border); padding:24px 18px; min-width:0; }
    main { min-width:0; padding:clamp(18px,3vw,34px); max-width:1440px; }
    h1 { margin:0 0 6px; font-size:clamp(34px,4.2vw,52px); letter-spacing:0; line-height:1; overflow-wrap:anywhere; }
    h2 { margin:0 0 18px; font-size:24px; line-height:1.15; overflow-wrap:anywhere; }
    p { color:var(--dim); line-height:1.5; }
    .brand { font-weight:800; font-size:20px; line-height:1.15; margin-bottom:28px; overflow-wrap:anywhere; }
    ...
    .card { background:var(--surface); border:1px solid var(--border); border-radius:10px; padding:22px; box-shadow:0 12px 34px rgba(18,24,38,.045); margin:22px 0; min-width:0; overflow:hidden; }
    ...
    th { color:var(--faint); font-size:12px; text-transform:uppercase; letter-spacing:.08em; white-space:normal; }
    td { overflow-wrap:anywhere; line-height:1.35; }
    ...
    .table-actions { display:flex; gap:8px; flex-wrap:wrap; min-width:150px; }
    ...
    .editor-grid { display:grid; grid-template-columns:minmax(240px,320px) minmax(0,1fr); gap:18px; align-items:start; }
    ...
    .setting-grid { display:grid; grid-template-columns:repeat(auto-fit,minmax(220px,1fr)); gap:16px; align-items:end; }
    ...
    .color-input input[type="color"] { width:56px; height:44px; padding:4px; }
    .color-input input[type="text"] { flex:1 1 auto; }
    @media (max-width:1150px) { .editor-grid { grid-template-columns:1fr; } .lesson-list { max-height:360px; } }
    @media (max-width:900px) { .shell { grid-template-columns:1fr; } aside { border-right:0; border-bottom:1px solid var(--border); } .nav { display:grid; grid-template-columns:repeat(auto-fit,minmax(120px,1fr)); gap:8px; } .toolbar { grid-template-columns:1fr; } main { padding:20px; } h1 { font-size:34px; } }
    @media (max-width:700px) { .form-grid { grid-template-columns:1fr; } .field-wide { grid-column:auto; } table { min-width:640px; } .card { padding:16px; } }
  </style>
</head>
<body>
...
const defaultAdminSettings = { theme:'light', accent:'#0f8a66', density:'comfortable' };
...
function safeAccent(value) {
  ...
  return /^#[0-9a-fA-F]{6}$/.test(color) ? color : defaultAdminSettings.accent;
}
function applyAdminSettings(settings = readAdminSettings()) {
  document.documentElement.style.setProperty('--accent', safeAccent(settings.accent));
...
async function api(path, options = {}) {
  ...
  try {
    ...
    return { ok:false, error:'network_error', message:'Could not reach the admin backend. Restart the local server and reload this page.' };
  }
}
...
document.getElementById('login-form').addEventListener('submit', async event => {
  ...
  if (!result.ok) { msg.textContent = result.message || result.error || 'Could not sign in.'; return; }
  showApp(result);
});
...
async function renderOverview() {
  ...
  const savedNotes = rows.reduce((s,r)=>s + Number(r.savedNoteCount||0),0);
  const renamed = rows.filter(r=>Number(r.nameChangeCount||0)>0).length;
  content.innerHTML = '<div class="notice"><strong>Backend-first V2.</strong> Data comes from PostgreSQL. V1 remains public while V2 is hidden.</div>' +
    ...
    '<div class="card stat"><strong>'+savedNotes+'</strong>Saved notes</div>' +
    '<div class="card stat"><strong>'+renamed+'</strong>Name-changed accounts</div>' +
    '</div>';
}
async function renderLearners() {
  title.textContent = 'Learners'; subtitle.textContent = 'Server-saved accounts, minutes, progress, Saved Notes, and name-change history.';
  const result = await api('/api/admin/learners');
  const rows = result.learners || [];
  content.innerHTML = '<div class="card"><div class="toolbar"><input id="search" placeholder="Search learners"><button id="refresh">Refresh</button></div><div class="table-wrap"><table><thead><tr><th>Name</th><th>Email</th><th>Minutes</th><th>Progress</th><th>Current</th><th>Visits</th><th>Saved notes</th><th>Name changes</th><th>Last active</th><th>Actions</th></tr></thead><tbody id="learner-rows"></tbody></table></div></div>';
  ...
  function draw(filter='') {
    tbody.innerHTML = rows.filter(r => (r.displayName + ' ' + r.email).toLowerCase().includes(filter.toLowerCase())).map(r => '<tr>'+
      '<td>'+esc(r.displayName)+(r.disabled?' <span class="pill">disabled</span>':'')+'</td><td>'+esc(r.email)+'</td><td>'+esc(r.totalMinutes)+'</td><td>'+esc(r.completionPercent)+'%</td><td>'+esc(r.currentLesson || '')+'</td><td>'+esc(r.visitCount)+'</td><td>'+esc(r.savedNoteCount || 0)+' / '+esc(r.toolkitCards || 0)+'</td><td>'+esc(r.nameChangeCount || 0)+'</td><td>'+esc(fmtDate(r.lastActiveAt))+'</td><td><div class="table-actions"><button class="secondary" data-detail="'+esc(r.id)+'">Details</button><button class="secondary" data-id="'+esc(r.id)+'" data-action="'+(r.disabled?'enable':'disable')+'">'+(r.disabled?'Enable':'Disable')+'</button></div></td></tr>').join('');
...
async function renderLearnerDetail(id) {
  ...
  const savedNotes = learner.toolkit || [];
  const nameStats = learner.nameChangeStats || {};
  const nameChanges = nameStats.recent || [];
  content.innerHTML =
    '<div class="card">' +
      ...
      '<div class="grid">' +
        ...
        '<div class="card stat"><strong>'+esc(savedNotes.length)+'</strong>Saved notes</div>' +
        '<div class="card stat"><strong>'+esc(nameStats.count || 0)+'</strong>Name changes</div>' +
      ...
      '</p>' +
    '</div>' +
    '<div class="card"><h2>Account history</h2><p class="muted">Name changes include learner edits from settings and admin support renames.</p><div class="table-wrap"><table><thead><tr><th>When</th><th>Source</th><th>Previous name</th><th>New name</th></tr></thead><tbody>'+nameChanges.map(row => '<tr><td>'+esc(fmtDate(row.changedAt))+'</td><td>'+esc(row.source || '')+'</td><td>'+esc(row.previousDisplayName || '')+'</td><td>'+esc(row.displayName || '')+'</td></tr>').join('')+'</tbody></table></div></div>' +
    '<div class="card"><h2>Saved Notes</h2><p class="muted">Notebook entries and optional saved lesson cards. Saving is never required to unlock the next activity.</p><div class="table-wrap"><table><thead><tr><th>When</th><th>Lesson</th><th>Type</th><th>Title</th></tr></thead><tbody>'+savedNotes.map(row => '<tr><td>'+esc(fmtDate(row.updatedAt || row.createdAt))+'</td><td>'+esc(row.lessonId || '')+'</td><td>'+esc(row.cardType || row.type || '')+'</td><td class="cell-title">'+esc(row.title || row.payload?.fields?.note || row.payload?.fields?.summary || '')+'</td></tr>').join('')+'</tbody></table></div></div>' +
...
function renderExport() {
  ...
  content.innerHTML =
    '<div class="grid">' +
      '<div class="card"><h2>Learner summary</h2><p>Accounts, minutes, progress, visits, current lesson, and last active time.</p><p><button data-export="/api/admin/export.csv" data-name="learning-ai-learners.csv">Download learner CSV</button></p></div>' +
      '<div class="card"><h2>Questionnaire answers</h2><p>Individual answers, age ranges, answer labels, scores, and timestamps.</p><p><button data-export="/api/admin/assessment-export.csv" data-name="learning-ai-questionnaire-answers.csv">Download questionnaire CSV</button></p></div>' +
    '</div>' +
    '<div class="card"><h2>Export notes</h2><p>Every export creates an audit event. CSV files may contain learner emails and questionnaire answer text, so keep them private.</p><p id="export-message" class="muted"></p></div>';
  content.querySelectorAll('button[data-export]').forEach(button => button.addEventListener('click', async () => {
    const msg = document.getElementById('export-message');
    msg.textContent = 'Preparing export...';
    const headers = {};
    if (csrfToken) headers['x-csrf-token'] = csrfToken;
    const res = await fetch(button.dataset.export, { method:'POST', headers, credentials:'include' });
    if (!res.ok) {
    ...
    a.href = url;
    a.download = button.dataset.name || 'learning-ai-export.csv';
...
function renderSettings() {
  ...
  const accents = [
    ['#0f8a66', 'Learning green'],
    ['#2563eb', 'Blue'],
    ['#0891b2', 'Teal'],
    ['#7c3aed', 'Violet'],
    ['#d97706', 'Amber']
  ];
  const currentAccent = safeAccent(settings.accent);
  content.innerHTML =
    '<div class="card">' +
      ...
      '<div class="setting-grid">' +
        ...
        '<div><span class="muted">Accent color</span><div class="swatches">' + accents.map(([color, label]) => '<button class="swatch'+(currentAccent === color ? ' active' : '')+'" data-accent="'+esc(color)+'" title="'+esc(label)+'" style="background:'+esc(color)+'"></button>').join('') + '</div></div>' +
        '<label><span>Custom accent</span><div class="color-input"><input id="admin-accent-color" type="color" value="'+esc(currentAccent)+'"><input id="admin-accent-text" value="'+esc(currentAccent)+'" spellcheck="false"></div></label>' +
      '</div>' +
  ...
  document.getElementById('admin-accent-color').addEventListener('input', event => {
    const accent = safeAccent(event.target.value);
    document.getElementById('admin-accent-text').value = accent;
    saveAdminSettings({ ...readAdminSettings(), accent });
  });
  document.getElementById('admin-accent-text').addEventListener('change', event => {
    const accent = safeAccent(event.target.value);
    saveAdminSettings({ ...readAdminSettings(), accent });
    renderSettings();
  });
  ...
  return http.createServer(async (req, res) => {
    ...
    try {
      ...
      if (req.method === 'GET' && url.pathname === '/api/admin/overview') {
        const session = await requireAdmin(req, res, database, { url });
        if (!session) return;
        const [learners, analytics, visits] = await Promise.all([
          database.adminLearners(),
          database.adminAssessmentAnalytics(),
          database.adminVisitAnalytics ? database.adminVisitAnalytics() : null
        ]);
        return sendJson(res, 200, { ok: true, ...buildAdminOverview({ learners, analytics, visits }) }, { req });
      }
      ...
      if (req.method === 'POST' && url.pathname === '/api/admin/assessment-export.csv') {
        const session = await requireAdmin(req, res, database, { csrf: true, url });
        if (!session) return;
        await database.audit({ adminUserId: session.admin.id, eventName: 'export_assessment_csv' });
        const rows = await database.exportAssessmentResponses();
        const csv = assessmentResponsesCsv(rows);
        res.writeHead(200, { 'content-type': 'text/csv; charset=utf-8', 'content-disposition': 'attachment; filename="learning-ai-questionnaire-answers.csv"', 'cache-control': 'no-store' });
        return res.end(csv);
      }
      ...
      if (req.method === 'PUT' && url.pathname === '/api/v2/profile') {
