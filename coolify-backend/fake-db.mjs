  ...
  return {
    ...
    async adminLearners() {
      return [...users.values()].map(row => {
        const cards = toolkit.filter(card => card.userId === row.id);
        ...
        return {
          ...
          displayName: row.display_name,
          totalMinutes: 0,
          visitCount: visits.filter(visit => visit.userId === row.id).length,
          completionPercent: 0,
          currentLesson: '',
          lastActiveAt: '',
          toolkitCards: cards.length,
          savedNoteCount: cards.filter(card => String(card.cardType || '').toLowerCase() === 'saved note').length,
    ...
    async exportAssessmentResponses() {
      return (await this.adminAssessmentAnalytics()).responses;
    },
    ...
    async saveToolkit(userId, { id, lessonId, cardType, payload }) {
      const existing = id ? toolkit.find(row => row.userId === userId && row.id === id) : null;
      if (existing) {
